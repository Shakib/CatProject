...
public class FirstClass_Katt {
	...
	public void testGetAmountOfTheAnimals_Horse() {
		Response response = get(baseURI + "/facts/random?animal_type=horse&amount=61");
		int amoun = response.jsonPath().getList(DEFAULT_BODY_ROOT_PATH).size();
		assertEquals(amoun, 61);	
	}
	
	@Test
	public void testGetAmountOfTheAnimals_Cat() {
		Response response = get(baseURI + "/facts/random?animal_type=cat&amount=453");
		int amoun = response.jsonPath().getList(DEFAULT_BODY_ROOT_PATH).size();
		assertEquals(amoun, 453);	
	}
	...
	@Test
	public void testGePendingFacts() {
		Response response = get(baseURI + "/facts");
		String pending = response.jsonPath().get("userUpvoted");
